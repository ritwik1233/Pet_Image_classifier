...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
       model_label = ""   
       ## Classify the image file and model and get model name   
       model_label=classifier(images_dir+key,model)
       ## edit the model label remove trailing space and convert the text to lower case
       model_label=model_label.lower().strip(' ')
       ## truth check the pet label and classifer label if match then add the classifier label and 1 in the list 
       ## else add classifier label and 0 
